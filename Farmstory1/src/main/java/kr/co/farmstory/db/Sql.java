...
public class Sql {
	...
	public static final String SELECT_COUNT_ARTICLES = "SELECT COUNT(*) FROM `JBOARD_ARTICLE` WHERE `title` is not null AND `cate`=? ;";
	
	public static final String SELECT_NOTICE = "(SELECT * FROM `JBOARD_ARTICLE` WHERE `cate`='notice' AND `parent`=0 ORDER BY `seq` DESC LIMIT 3) "
												+ "UNION "
